(function() {
  ...
  var connect = function(params, url, cb) {
    ...
    handshakeBuffer.sys.cid = challengeId;
    handshakeBuffer.sys.sid = clientId;
    ...
    var onclose = function(event) {
      ...
      if(!!params.reconnect && !reconnectPurged && reconnectAttempts < maxReconnectAttempts) {
        reconnect = true;
        reconnectAttempts++;
  ...
  var close = pomelo.disconnect = function() {
    purge();
    if(socket) {
  ...
  var purge = function(){
    reconnectPurged = true; //purge reconnect here
    clientId = ''; //reset client which would not use any more
    challengeId = (new Date()).valueOf() + ''; //gen a new challengeId for next time use
  }

  var reset = function() {
    reconnectPurged = false;
    reconnect = false;
  ...
  var handshake = function(data) {
    ...
    if(data.code !== RES_OK) {
      if(data.code === RES_OLD_CLIENT) {
        data.message = 'client version not fullfill'
        pomelo.emit('error', data);
      }
      else{
        if(!data.message)
          data.message = 'handshake fail'
        pomelo.emit('error', data);
      }

      if(initCallback) {
        initCallback(data); //let user know the error
      }
    ...
    send(obj);
    if(initCallback) {
      initCallback(null, data.user); //let user know the user result
    }
  };
